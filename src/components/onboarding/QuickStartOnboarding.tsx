  ...
  const saveProvider = async () => {
    ...
    try {
      ...
      const definition = await providerCenterApi.save({
        ...
        notes: t("quickStart.providerNotes"),
        enabled: true,
        credentialAction: "replace",
        apiKey: cleanKey,
        appTypes: ["codex"],
      });
      const preview = await providerCenterApi.previewApply(definition.id, [
        "codex",
      ]);
      ...
      if (result.status !== "applied") {
        throw new Error(
          t("quickStart.applyIncomplete", { status: result.status }),
        );
      }
          ...
          <section className="relative overflow-hidden border-b border-border/70 bg-muted/30 p-7 md:border-b-0 md:border-r md:p-10">
            ...
            <div className="relative flex h-full flex-col">
              ...
              <div className="mt-8 space-y-3">
                {["stepAssistant", "stepProvider", "stepHelp"].map(
                  (key, index) => (
          ...
          <section className="p-7 md:p-10">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium">
                    {t("quickStart.codexCli")}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {cliReady
                      ? t("quickStart.cliReady", { version: runtime?.version })
                      : t("quickStart.cliHint")}
                  </p>
                </div>
                {!cliReady && (
                  <Button
                    ...
                    disabled={checkingRuntime || installing}
                  >
                    {installing ? (
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="quick-provider-name">
                  {t("quickStart.providerName")}
                </Label>
                <Input
                  id="quick-provider-name"
                  value={name}
                  onChange={(event) => setName(event.target.value)}
                  placeholder={t("quickStart.providerNamePlaceholder")}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="quick-provider-url">
                  {t("quickStart.baseUrl")}
                </Label>
                <Input
                  id="quick-provider-url"
                  value={baseUrl}
              ...
              <div className="space-y-2">
                <Label htmlFor="quick-provider-key">
                  {t("quickStart.apiKey")}
                </Label>
                <div className="relative">
                  <KeyRound className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                  <Input
                    id="quick-provider-key"
                    type="password"
                    value={apiKey}
            ...
            <Collapsible
              ...
              className="mt-5 rounded-xl border border-border/70"
            >
              ...
              <CollapsibleContent className="border-t border-border/70 px-4 pb-4 pt-4">
                <p className="mb-4 text-xs leading-5 text-muted-foreground">
                  {t("quickStart.advancedHint")}
                </p>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="quick-protocol">
                      {t("quickStart.protocol")}
                    </Label>
                    <select
                      id="quick-protocol"
                      value={apiFormat}
                      onChange={(event) =>
                        setApiFormat(event.target.value as ApiFormat)
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder={t("quickStart.modelPlaceholder")}
                    />
                  </div>
                </div>
              </CollapsibleContent>
