...
pub async fn update_provider(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let state = app_handle
            .try_state::<AppState>()
            .ok_or_else(|| "应用状态不可用".to_string())?;
        let guarded_id = originalId.as_deref().unwrap_or(&provider.id);
        crate::provider_center::guard_projection_mutation(state.inner(), &app_type, guarded_id)
            .map_err(|e| e.to_string())?;
        if provider.id != guarded_id {
            crate::provider_center::guard_projection_mutation(
                state.inner(),
                &app_type,
                &provider.id,
            )
...
pub fn delete_provider(
    ...
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
    crate::provider_center::guard_projection_mutation(state.inner(), &app_type, &id)
        .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn set_provider_live_enabled(
    app_handle: tauri::AppHandle,
    app: String,
